...
class Message(object):
    ...
    __slots__ = ["type", "flags", "seq", "data", "datasize", "hmac"]

    def __init__(self, msgType, databytes, flags, seq):
        self.type = msgType
        self.flags = flags
        self.seq = seq
        self.data = databytes
    ...
    def __header_bytes(self):
        checksum = (self.type+constants.PROTOCOL_VERSION+self.datasize+self.flags+self.seq+CHECKSUM_MAGIC)&0xffff
        return struct.pack(header_format, b"PYRO", constants.PROTOCOL_VERSION, self.type, self.flags, self.seq, len(self.data), checksum, self.hmac)

    @classmethod
    def from_header(cls, headerData):
        """Parses a message header. Does not yet process the message data."""
        if not headerData or len(headerData)!=HEADERSIZE:
            raise errors.ProtocolError("header data size mismatch")
        tag, ver, msgType, flags, seq, dataLen, headerchecksum, datahmac = struct.unpack(header_format, headerData)
        if tag!=b"PYRO" or ver!=constants.PROTOCOL_VERSION:
            raise errors.ProtocolError("invalid data or unsupported protocol version")
        if headerchecksum!=(msgType+ver+dataLen+flags+seq+CHECKSUM_MAGIC)&0xffff:
            raise errors.ProtocolError("header checksum mismatch")
        msg = Message(msgType, b"", flags, seq)
    ...
    def recv(cls, connection, requiredMsgTypes=None):
        ...
        headerdata = connection.recv(HEADERSIZE)
