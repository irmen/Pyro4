...
THREADPOOL_ALLOW_QUEUE  bool    False          When no free threads: allow new connections to be put into a wait queue (old behavior), or return connection error (new default behavior) *note: will disappear again when different thread pool implementation is shipped in a future release*
